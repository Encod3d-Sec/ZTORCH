...
qmd is a CLI + MCP server that provides semantic and keyword search over `wiki/`. The MCP server (`wiki-search`) exposes two tools consumed by the agent (ZCode) directly. The CLI is used for maintenance (re-index, status).
...
```bash
export QMD_VAULT="/path/to/your/ObisidianVaults/ZTorch/ZTorch"
```
...
AGENTS.md rule: **never read `wiki/index.md` to find pages - always search first.**
