...
- **Main brain - GLM (the plan's flagship):** the `campaign.py` loop, board/state, strategy, Burp/chrome-devtools, finding write-ups. This session.
- **Verifier - GLM (fresh context), MANDATORY:** before any CONFIRMED, `campaign.py verify <F>` prints a refuter prompt; dispatch ONE fresh agent on the main tier that reads the raw PoC, tries to REFUTE, and writes `verdicts/<F>.json`. `done --find` refuses unless that verdict exists, is `refuted:false`, and cites the finding's PoC (anti-rubber-stamp). Fails CLOSED.
- **RTL - GLM (fresh context):** `Skill(redteamlead)` for direction at a fork or when a vector stalls.
- **Short tasks - GLM-Flash (cheap tier):** `Skill(wiki-arsenal)` deep, `Skill(delegate)` (mechanical exploit-run), `Skill(ingest)` recon-parse. Bounded, fully-specified, single-shot ONLY.

The line that keeps quota safety: GLM-Flash is allowed for a **bounded single job**, never for open-ended parallel hunting.
...
- **Do NOT** invoke `superpowers:brainstorming` or `superpowers:writing-plans` mid-campaign, and keep
  no parallel `TodoWrite` list. The board is the plan.
- Model routing (see `## Model routing`): the GLM flagship drives the loop; bounded short tasks go to GLM-Flash;
  the mandatory main-tier **verifier** gates every finding (`done --find` refuses without a passing
  `verdicts/<F>.json`). Still **no open-ended hunter fan-out** (it exhausted the weekly quota last
  time) - GLM-Flash is only for single, fully-specified jobs.
