...
# so its VAULT global self-locates to the REAL vault while ZTORCH_VAULT is still
...
def _env(vault):
    return dict(os.environ, ZTORCH_VAULT=str(vault))
...
def test_close_out_autobuilds_walkthrough_on_solved(vault):
    ...
    # realpath still resolves _engagement from the real repo, and it reads ZTORCH_VAULT).
    sdir = vault / "scripts"
