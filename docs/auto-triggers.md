...
- hook registration: committed `<vault>/.zcode/config.json` (ZCode workspace scope; `setup/install-hooks.sh` verifies it)
...
Nothing runs a command on its own. To change what fires: edit `triggers.json` (keywords), `playbook.json` (fingerprints + tests), or the hook `.py` files; the registration itself lives in the committed `.zcode/config.json` (verify with `bash setup/install-hooks.sh`).

---

## ZCode hook mechanics (output contract)

ZCode supports exactly seven events (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PostToolUseFailure`, `Stop`); the old PreCompact slot has no equivalent, so `pre-compact.sh` sits unregistered in `skills/hooks/` (ZCode compaction preserves continuity through the SessionStart hooks instead). Registration: the committed `.zcode/config.json` -> `hooks.events` (`hooks.enabled: true` required -- configuration hooks are off by default); commands resolve scripts via the `${ZCODE_PROJECT_DIR}` template var; matchers are case-sensitive regexes on the tool name (`Bash`, `Write|Edit`, ...).
...
- Advisory guards (`drift-guard`, `session-guard`, `recon-capture`) emit `additionalContext` only; enforcement (`scope-guard`) emits `permissionDecision: "deny"` with a reason.
