...
**Engagement workflow (the driver is the plan).** For any offensive engagement run `Skill(engage)` first (the router), then `Skill(offensive)` (`scripts/offensive.py --type pentest|bb|ctf`). The deterministic driver `scripts/offensive.py` owns the coverage board, generates it from recon, and prints the exact next action every turn. It is the single source of truth for this section: it ENFORCES G1 arsenal-first, G2 skill-first, G3 typed evidence, G4 deadend-first, G5 depth-first, G7 no-ask, G8 tool-first, plus pre-board recon, the read-whole gate, the effort-ceiling stop, OOB and two-account readiness, ban control, and budget/report-only - and its `next`/`foothold` output carries the full copy-paste command and the poll-on-condition / delegate-at-foothold reflexes. Follow its output literally. Health check: `offensive-doctor` (`scripts/offensive-doctor.py`).
...
- **RE before brute-guess.** A custom/unknown binary or protocol with no public exploit (a signal generator, a keygen, a clearance/HMAC check) - read/disassemble it BEFORE brute-guessing the value it computes. Guessing a small output space is rarely faster than reading a small readable/unstripped binary (recurring misses: ~96 HMAC guesses before decompiling 2 tiny functions that gave the key directly; 3 wrong magic-signal retries before reading an unstripped kernel module).
...
- SessionStart: `engagement-init.py` (self-heal engagement files; inject state summary + next-moves + OOB HITs), `session-start.sh` (register skills; session state; rotates `session/hot.md` to ~3 newest entries, older archived verbatim to `session/hot-archive.md`).
