...
- **Delegate exploit EXECUTION to a cheap-tier sub-agent (cost control, default).** The main model (GLM) stays on strategy, the board, wiki-first analysis, triage, and next-move; the fully-specified exploit/PoC/escalation RUN (copy-paste run, compile, spray, shell-driving, endpoint grinding) goes to a cheap-tier sub-agent (GLM-Flash) via `Skill(delegate)` - dispatch, wait (no parallel duplicate), integrate the result. Don't burn main-model tokens hand-running exploit loops yourself. When the hand-off reads source/config, spell out read-first IN THE PROMPT (full `cat` of each file, grep only to locate) - a cheap model grep-mines by default and your prompt is its only law. And a sub-agent's NEGATIVE ("empty/dead/rabbit-hole") is a HYPOTHESIS, not a fact: re-verify it yourself before it becomes a hard `[!]`/Deadends exclusion, doubly so when it gates the last open vector - a cheap model's "empty" is often a query/tooling mistake (a real miss: a sub-agent's "UCP voicemail empty" - it hit the AJAX endpoint, not the dashboard WIDGET that rendered the secret - got fossilized into "UCP=rabbit hole" and cost an hour+ chasing a nonexistent pspy/cron path). Delegate MECHANICAL/wait-heavy runs (pspy wait, brute, fixed-payload flag-read) to the cheap tier; keep JUDGMENT-heavy enumeration (route/token discovery, "is this empty or am I querying it wrong") on the main model.
...
Output/mode plugins: **ponytail** (lazy-code discipline - YAGNI, stdlib/native first, shortest working diff) is an optional Claude-marketplace plugin; if installed for ZCode (Settings -> Plugin Management; `.claude-plugin` manifests are recognized) it auto-activates at SessionStart via its own hook (level `full`; switch with `/ponytail lite|full|ultra`), and governs what you build, not prose. **caveman** (prose compression - terse output, drop articles/filler, fragments OK) is manual per session via `/caveman`, same optional install path. Neither is required: the harness degrades gracefully without them.

SessionStart also auto-loads `session/hot.md`. No manual reads needed.

Engagement-state hooks (workspace-scoped, registered in the committed `.zcode/config.json`, scripts under `skills/hooks/`). All fail open (any error -> allow, never trap). Policy: **deterministic guards ENFORCE (deny the tool call); semantic reflexes ADVISE (inject a suggestion).** Enforcement is reserved for no-judgement checks (scope/RoE) where blocking the wrong action costs zero tokens; judgement calls (wiki-first, tools-not-manual, intended-path) stay advisory because a false block wastes more time than it saves. Escape hatch for a bad block: `touch skills/hooks/.enforce-off`. ZCode supports exactly seven hook events (`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PostToolUseFailure`, `Stop`); the old `PreCompact` slot (`pre-compact.sh`) has no ZCode equivalent and stays unregistered. Full mechanics in `docs/auto-triggers.md`; the behaviorally-relevant summary:
...
| `drift-guard.py` | PreToolUse/Bash | Nudges when shell work drifts off the `Approach.md` plan board. Advisory. |
| `capture-poc.py` / `tool-telemetry.py` / `wiki-reindex.py` | PostToolUse | PoC-pair capture, leak-safe tool telemetry, qmd re-index after vault writes. |
| `close-out.py` | Stop | Session-close reminder (pause-work split: generic vs client data). |
...
Per-machine hostnames and vault paths live in the git-ignored `AGENTS.local.md`
(copy `AGENTS.local.example.md` to create it), kept out of the published repo.
Read it when a path in it is needed or resolution fails; do not rely on an
automatic import (ZCode does not expand `@file` includes in AGENTS.md).
The path resolvers (`setup/vault-path.sh`) and hooks self-locate or read
`ZTORCH_VAULT` / `OBSIDIAN_VAULT` / `QMD_VAULT`, so a single-machine setup needs
no local file.
...
```
ZTORCH/
├── AGENTS.md   <- this file (+ README.md, LICENSE)
├── .zcode/     <- config.json (hook registration, committed) · skills/ (per-machine links, ignored)
