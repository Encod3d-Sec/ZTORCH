...
type: technique
tags: [active-directory, adcs, certificates, enumeration, reference-import, windows]
phase: enumeration
...
AD CS issues X.509 certificates that domain members use for authentication, encryption, and code signing; because certificates authenticate to Kerberos via PKINIT, a compromised or forged certificate is equivalent to a long-lived password. Attackers enumerate the CA infrastructure and certificate templates to identify misconfigurations, then exploit them to enroll in templates that allow privilege escalation or impersonation. The persistence value is high because certificates remain valid after password resets and can be used to re-obtain Kerberos TGTs on demand.
