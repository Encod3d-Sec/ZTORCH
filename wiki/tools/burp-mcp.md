...
client (ZCode, Claude Desktop, any MCP-capable client) can drive Burp directly: read proxy history,
...
### Native mode (MCP client running on Kali)

The client speaks stdio, Burp speaks SSE, so a small proxy jar bridges them. Create
...
### Bridge mode (agent on the host, Burp on Kali)

Default for this vault, because ZCode runs on the Windows/WSL host and reaches
